...
export const waitlistRouter = createTRPCRouter({
    ...
    create: baseProcedure
        ...
        .mutation(async ({ input }) => {
            ...
            const existingEntry = await db.select().from(waitlist).where(eq(waitlist.email, input.email));
            ...
            const [createdWaitlistEntry] = await db.insert(waitlist)
                .values({
                    email: input.email,
                })
                .returning();
            ...
            try {
                await resend.emails.send({
                    ...
                    to: input.email,
                    subject: "🎉 Welcome to the Pagr Waitlist!",
